...
function findMedia(data){
    ...
    for(var i = 0; i < Exts.length; i++){
        ...
        if(data_ext.indexOf(ext)== -1){
            continue;
        }else if(Exts[i].size == 0 || size >= Exts[i].size*1024*1024 || size == null){
            filter = true;
            break;
        }else{
            return;
    ...
    if(filter){
        ...
        size = size  +"MB";
        var info={name:name,url:url,size:size,ext:ext,type:contentType,tabid:data.tabId,title:title,port:port};
